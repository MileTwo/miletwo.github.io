...
// assign the default open tab for job postings
$(document).ready(() => {
    // Grab the current URL
    let url = document.URL;
    let targetJob = url.split('/')[4].replace('#', '');

    let i, tablinks;
    tablinks = document.getElementsByClassName("tablinks");

    for (i = 0; i < tablinks.length; i++) {
        let tablinkAddress = tablinks[i].href;
        let job = tablinkAddress.split('#')[1].replace('/', '');
        if (targetJob === job) {
            // Assign the default open tab to the target job from the URL
            tablinks[i].id = 'defaultOpen';
        } else if (targetJob === '') {
            // Assign the default open tab to the first job opening
            tablinks[0].id = 'defaultOpen';
        }
    }
});

// tab navigation for job openings
$(document).ready(function () {
    $(".tablinks").click(e => {
        let url = e.target.hash;
        let position = url.replace("#", "").replace("/", "");
